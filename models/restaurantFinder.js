module.exports = ([
    {
        city: 'Paris',
        restaurants: [
            {
                name: "Septime",
                address: '80 Rue de Charonne, 75011 Paris, France',
                website: 'https://www.septime-charonne.fr/',
                description: 'Refined tasting menus featuring creative dishes paired with wines, in a minimalist space.',
                img: 'https://axwwgrkdco.cloudimg.io/v7/__gmpics__/91fa3530369e4e9ba284384acb08e9a8',
                type: ['French', 'Mediteranian', 'Small Plates'],
                rating: 4.5
            },
            {
                name: "Restaurant L'Ange 20",
                address: '44 Rue des Tournelles, 75004 Paris, France',
                website: 'https://www.lange20.com/',
                description: 'Home-style French dishes & a selection of local wines in a charming venue with terrace seating.',
                img: 'https://res.cloudinary.com/tf-lab/image/upload/restaurant/855d383a-eefe-4ce2-bcb9-7ddafdcba409/daef0e5d-dc68-4afc-a24e-78488b62cdef.jpg',
                type: ['French', 'Mediteranian', 'Small Plates'],
                rating: 4.7
            },
            {
                name: "La Table de Colette",
                address: '17 Rue Laplace, 75005 Paris, France',
                website: 'https://www.latabledecolette.fr/',
                description: 'Fine dining, eco-responsible restaurant with a terrace serving 3- to 7-course menus & wine pairings.',
                img: 'https://www.latabledecolette.fr/i/la-table-de-colette-479852/3/5/3/2/8/4/1/5/0/8/5/9/3/1591629635_445/0e83bbc2300b478d34e1e192d6870907.jpg',
                type: ['Haute French Restaurant', 'Small Plates'],
                rating: 4.8
            },
            {
                name: "Restaurant Guy Savoy",
                address: 'Monnaie de Paris, 11 Quai de Conti, 75006 Paris, France',
                website: 'https://www.guysavoy.com/',
                description: 'Sophisticated restaurant offering high-concept a la carte & fixed-price menus, plus wine pairings.',
                img: 'https://res.cloudinary.com/neonfeast/image/upload/c_limit,w_1200/v1641232805/sys/listings/5f1623f7-0444-4ae1-b512-3fe6288ff55b/restaurant_guy_savoy_terrace_room.jpg',
                type: ['Haute French Restaurant', 'Small Plates'],
                rating: 4.6
            },
            {
                name: "Restaurant Eels",
                address: `27 Rue d'Hauteville, 75010 Paris, France`,
                website: 'https://www.restaurant-eels.com/',
                description: 'Neo-bistro delivering creative French dishes & tasting menus in a cool, pared-back space.',
                img: 'https://axwwgrkdco.cloudimg.io/v7/__gmpics__/ba42d98a3f4f4efba9cb61f34165b109',
                type: ['French Restaurant', 'Small Plates', 'Mediterenian Cousine'],
                rating: 4.8
            },
            {
                name: "Frenchie",
                address: `5 Rue du Nil, 75002 Paris, France`,
                website: 'http://www.frenchie-ruedunil.com/',
                description: `Rustic-chic stop for seasonal French dishes with global influences, plus chef's carte blanche menus.`,
                img: 'https://www.theworlds50best.com/discovery/filestore/jpg/Frenchie-Paris-France-01.jpg',
                type: ['French Restaurant', 'Small Plates', 'Mediterenian Cousine'],
                rating: 4.5
            }
        ]
    },
    {
        city: 'New York',
        restaurants: [
            {
                name: "Eleven Madison Park",
                address: '11 Madison Ave, New York, NY 10010',
                website: 'https://www.elevenmadisonpark.com/',
                description: 'Upscale American tasting menus from chef Daniel Humm served in a high-ceilinged art deco space.',
                img: 'https://cdn.vox-cdn.com/thumbor/XW2uJVJVPjPiRPUAATN3J9qji3c=/0x0:5412x3608/1200x800/filters:focal(2274x1372:3138x2236)/cdn.vox-cdn.com/uploads/chorus_image/image/62861527/AC1I0203.0.jpg',
                type: ['Fine Dining Restaurant', 'American'],
                rating: 4.5
            },
            {
                name: "STK Steakhouse Midtown NYC",
                address: '1114 6th Ave, New York, NY 10036',
                website: 'https://stksteakhouse.com/venues/nyc-midtown/',
                description: `Sceney spot that's both a chic lounge with DJs & a modern steakhouse serving prime cuts & seafood.`,
                img: 'https://stksteakhouse.com/wp-content/uploads/2018/11/ny-midtown-overview.jpg',
                type: ['Fine Dining Restaurant', 'Steak House'],
                rating: 4.6
            },
            {
                name: "Olio e Più",
                address: '3 Greenwich Ave, New York, NY 10014',
                website: 'https://www.olioepiu.com/',
                description: `Naples meets NYC at this trattoria with thin-crust pizza, Italian wines & ample sidewalk seating.`,
                img: 'https://whatnowchicago.com/wp-content/uploads/sites/13/2022/06/My-project-2022-06-27T190353.868.jpg',
                type: ['Italian', 'Pizza', 'Pasta'],
                rating: 4.5
            },
            {
                name: "Boucherie West Village",
                address: '99 7th Ave S, New York, NY 10014',
                website: 'https://www.boucherie.nyc/',
                description: `Two-floor bistro for dry-aged steaks & other French fare, with a bar that's strong on absinthe.`,
                img: 'https://media-cdn.tripadvisor.com/media/photo-s/0f/4b/bd/55/ground-floor.jpg',
                type: ['French Restaurant'],
                rating: 4.5
            },
            {
                name: "Le Coucou",
                address: '99 7th Ave S, New York, NY 10014',
                website: 'https://lecoucou.com/',
                description: `Refined French fare, such as lobster, rabbit, duck & foie gras, served in an airy, elegant setting.`,
                img: 'https://cdn.vox-cdn.com/thumbor/UEBFdkLwPR3ywSoEMwcjdRHyiT8=/0x0:2000x1335/1200x800/filters:focal(840x508:1160x828)/cdn.vox-cdn.com/uploads/chorus_image/image/70935735/2019_10_07_SiblingRival_005.0.jpg',
                type: ['French Restaurant'],
                rating: 4.5
            },
            {
                name: "La Grande Boucherie",
                address: '145 W 53rd St, New York, NY 10019',
                website: 'https://www.boucherie.nyc/',
                description: `Polished French restaurant in an expansive art nouveau-style space with an elegant heated atrium.`,
                img: 'https://images.getbento.com/accounts/517bd9e542125bd665a34f65494870d7/media/images/56348northroom2.JPG?w=1200&fit=crop&auto=compress,format&h=600',
                type: ['French Restaurant'],
                rating: 4.5
            }
        ]
    },
    {
        city: 'Los Angeles',
        restaurants: [
            {
                name: "République Café Bakery & République Restaurant",
                address: '624 S La Brea Ave, Los Angeles, CA 90036',
                website: 'https://republiquela.com/',
                description: `Modern French plates served in a striking space with communal tables, plus a bakery & cocktail bar.`,
                img: 'https://media.architecturaldigest.com/photos/55f9e1524254f7de3455ed52/2:1/w_544,h_272,c_limit/dam-images-daily-2014-03-republique-la-republique-los-angeles-01-interior-dining-area.jpg',
                type: ['Modern French Restaurant'],
                rating: 4.6
            },
            {
                name: "Otium",
                address: ' 222 S Hope St, Los Angeles, CA 90012',
                website: 'https://otiumla.com/',
                description: `New American cuisine, cocktails & high design are offered in an airy space next to The Broad museum.`,
                img: 'https://images.otstatic.com/prod/24590164/1/large.jpg',
                type: ['New American Restaurant'],
                rating: 4.5
            },
            {
                name: "Damian",
                address: '2132 E 7th Pl, Los Angeles, CA 90021',
                website: 'https://damiandtla.com/',
                description: `Polished eatery making Mexican dishes & desserts with locally sourced ingredients, plus cocktails.`,
                img: 'https://assets3.thrillist.com/v1/image/3067620/828x610/flatten;crop;webp=auto;jpeg_quality=60.jpg',
                type: ['Mexican Restaurant'],
                rating: 4.6
            },
            {
                name: "Yard House",
                address: '800 W Olympic Blvd A-115, Los Angeles, CA 90015',
                website: 'https://yardhouse.com/',
                description: `High-end sports-bar chain with a huge menu of New American fare & an extensive list of draft beers.`,
                img: 'https://cdn.usarestaurants.info/assets/uploads/6cab0824c0ddb39d0fe025634f490642_-united-states-california-los-angeles-county-burbank-142571-yard-househtm.jpg',
                type: ['New American Restaurant'],
                rating: 4.4
            },
            {
                name: "FIN",
                address: '12223 W Washington Blvd, Los Angeles, CA 90066',
                website: 'https://finasiantapas.com/home/',
                description: `Sparely decorated restaurant featuring Japanese & American fare & booze in a candlelit setting.`,
                img: 'https://columbialivingmag.com/img/columbia/site/Food/bluefin3.jpg',
                type: ['Asian Fusion Restaurant'],
                rating: 4.4
            },
            {
                name: "La Boucherie",
                address: '900 Wilshire Blvd fl 71, Los Angeles, CA 90017',
                website: 'https://downtownla.com/go/la-boucherie/',
                description: `French meets California chophouse on top of the InterContinental hotel, with jaw-dropping views.`,
                img: 'https://img.ctykit.com/cdn/ca-dtla/images/tr:w-1800/laboucherie.png',
                type: ['Steak House'],
                rating: 4.5
            }
        ]
    },
    {   
        city: `Sydney`,
        name: `Aria Restaurant Sydney`,
        address: `1 Macquire St, Sydney NSW 2000, Australia`,
        website: `https://www.ariasydney.com.au/`,
        description: `Chef Matt Moran presents Modern Australian haute cuisine in an elegant, glass-walled dining room.`,
        img: `https://media-cdn.tripadvisor.com/media/photo-s/18/01/b6/1c/aria-sydney.jpg`,
        type: [
            `Fine dining`,
            `Modern Australian haute cuisine`
        ],
        rating: 4.5
    },
    {
        city: `Sydney`,
        name: `Restaurant Hubert`,
        address: `15 Bligh St, Sydney NSW 2000, Australia`,
        website: `https://www.swillhouse.com/`,
        description: `Classic French dishes showcased in a sophisticated venue featuring a lively bar & vintage decor.`,
        img: `https://media.cntraveler.com/photos/59c946a67448196e0e771a95/16:9/w_2560,c_limit/cr-hubert.jpg`,
        type: [
            `French Restaurant`,
            `Vintage`
        ],
        rating: 4.6
    },
    {
        city: `Sydney`,
        name: `Long Chim Sydney`,
        address: `Corner of Pitt St and Angel Place, Angel Pl, Sydney NSW 2000, Australia`,
        website: `https://www.longchimsydney.com/`,
        description: `Hip eatery in an industrial space serving Thai classics including curries, stir fries & meat dishes.`,
        img: `https://images.squarespace-cdn.com/content/v1/5ed465331aadaa1625c491f7/1591177799066-V4BWSQA1B036PXSMEKNP/long+chim+interior.jpg?format=2500w`,
        type: [
            `Thai Restaurant`,
            `Hip`
        ],
        rating: 4.1
    },
    {
        city: `Sydney`,
        name: `Chophouse Sydney`,
        address: `25 Bligh St, Sydney NSW 2000, Australia`,
        website: `https://www.chophousesydney.com.au/`,
        description: `A high-end, contemporary steakhouse dishing up premium meats in a warmly lit vintage space.`,
        img: `https://youimg1.tripcdn.com/target/100c0f0000007ddn49416.jpg`,
        type: [
            `Steakhouse`,
            `Vintage`
        ],
        rating: 4.3
    },
    {
        city: `Sydney`,
        name: `Bistro Papillon`,
        address: `98 Clarence St, Sydney NSW 2000, Australia`,
        website: `https://www.bistropapillon.com.au/`,
        description: `Onion soup, coq au vin and other French dishes in a cosy, romantic space decorated with old posters.`,
        img: `https://i0.wp.com/www.bistropapillon.com.au/wp-content/uploads/2020/05/PrivateParty__0005_A42I2569.jpg?resize=1000%2C667&ssl=1`,
        type: [
            `French Restaurant`,
            `Romantic`
        ],
        rating: 4.5
    },
    {
        city: `Sydney`,
        name: `NOMAD Sydney`,
        address: `16 Foster St, Surry Hills NSW 2010, Australia`,
        website: `https://nomad.sydney/`,
        description: `Upbeat restaurant in a converted warehouse serving contemporary cuisine with Middle Eastern accents.`,
        img: `https://media.timeout.com/images/105917669/750/422/image.jpg`,
        type: [
            `Australian Restaurant`,
            `Converted Warehouse`
        ],
        rating: 4.6
    },
    {
        city: `London`,
        restaurants: [
            {
                name: `Twist Connubio`,
                address: `42 Crawford St, London W1H 1JW, United Kingdom`,
                website: `https://www.twistconnubio.com/`,
                description: `Rustic-chic dining room serving international, ingredient-led small plates cooked on a Josper grill.`,
                img: `https://www.hardens.com/restaurant-images/UNADJUSTEDNONRAW_thumb_1524/RG-17fd-8571-mp.jpg`,
                type: [
                    `Small plate`,
                    `Rustic-chic`
                ],
                rating: 4.6
            },
            {
                name: `Berners Tavern`,
                address: `10 Berners St, London W1T 3NP, United Kingdom`,
                website: `http://www.bernerstavern.com/`,
                description: `Jason Atherton's upmarket Modern British restaurant in Ian Schrager's London Edition Hotel.`,
                img: ``,
                type: [
                    `European Restaurant`,
                    `Modern`
                ],
                rating: 4.5
            },
            {
                name: `Aquavit London`,
                address: `1 St James's Market, St. James's, London Sw1Y 4QQ, United Kingdom`,
                website: `https://www.aquavitrestaurants.com/`,
                description: `Polished eatery spotlighting elevated Scandinavian cuisine & desserts, plus a robust wine list.`,
                img: `https://keyassets.timeincuk.net/inspirewp/live/wp-content/uploads/sites/34/2017/05/Aquavit-restaurant-London-interior.jpg`,
                type: [
                    `Scandinavian Restaurant`,
                    `Robust wine list`
                ],
                rating: 4.4
            },
            {
                name: `Jikoni`,
                address: `19-21 Blandford St, London W1U 3DJ, United Kingdom`,
                website: `https://www.jikonilondon.com/`,
                description: `Cosy kitchen-style diner serving Asian comfort food with British, African and Middle Eastern twists.`,
                img: `https://resizer.otstatic.com/v2/photos/wide-large/1/26030092.jpg`,
                type: [
                    `Asian fusion restaurant`,
                    `British, African and Middle Eastern twists`
                ],
                rating: 4.5
            },
            {
                name: `Launceston Place`,
                address: `1A Launceston Pl, London W8 5RL, United Kingdom`,
                website: `https://www.launcestonplace-restaurant.co.uk/`,
                description: `Fine dining restaurant in an 1839 townhouse, with a notable wine list and Modern British menu.`,
                img: `https://resizer.otstatic.com/v2/photos/wide-xlarge/1/25542340.jpg`,
                type: [
                    `Modern European Restaurant`,
                    `Fine dining`
                ],
                rating: 4.8
            },
            {
                name: `Lyle's`,
                address: `Tea Building, 56 Shoreditch High St, London E1 6GY, United Kingdom`,
                website: `https://www.lyleslondon.com/`,
                description: `This stripped back dining room serves seasonal British fare as a la carte lunches and set suppers.`,
                img: `https://www.theworlds50best.com/filestore/jpg/W50BR2021-150-Lyle's-1.jpg`,
                type: [
                    `European Restaurant`,
                    `Modern/Industrial`
                ],
                rating: 4.5
            }
        ]
    },
    {
        city: `Dubai`,
        restaurants: [
            {
                name: `Miss Tess`,
                address: `Taj Dubai - Burj Khalifa St - Business Bay - Dubai - United Arab Emirates`,
                website: `https://www.misstessdubai.com/`,
                description: `Asian Street Food, daily shows, and delicious cocktails! Miss Tess is an Asian street-food restaurant inspired by a story.`,
                img: `https://www.timeoutdubai.com/cloud/timeoutdubai/2021/09/12/XumtkjIw-Miss-Tess-1200x800.jpg`,
                type: [
                    `Asian street-food`,
                    `Experience`
                ],
                rating: 4.3
            },
            {
                name: `Günaydin`,
                address: `Souk Al Bahar - Downtown Dubai - Dubai - United Arab Emirates`,
                website: `https://www.gunaydindubai.com/`,
                description: `Set in the beautiful Souk Al Bahar development, Günaydin Meatery Dubai sits on the waterfront promenade, enchanting guests with a spectacular views of the world-famous Dubai Fountain and the Burj Khalifa.`,
                img: `https://b.zmtcdn.com/data/pictures/3/18410863/2cdabe1ef0826c86888d90156bd6d502.jpg`,
                type: [
                    `Turkish Restaurant`,
                    `High quality meats`
                ],
                rating: 4.2
            },
            {
                name: `Tribes Dubai Mall`,
                address: `Level 2, Star Atrium, above apple store, The Dubai Mall - Dubai - United Arab Emirates`,
                website: `https://www.tribesrestaurant.com/`,
                description: `Traditional & fusion dishes from African countries highlighted in a stylish venue with a terrace.`,
                img: `https://www.ramadigroup.com/wp-content/uploads/75efabd8a2075f0cbf30e5f2ff052bc9.jpg`,
                type: [
                    `African Restaurant`,
                    `Fushion`
                ],
                rating: 4.5
            },
            {
                name: `Tulum Restaurant`,
                address: `The Dubai Mall Unit No. 4F - 04, Fashion Avenue Extention - Dubai - United Arab Emirates`,
                website: `https://tulum.ae/`,
                description: `Classic Mexican cooking with a twist is served at this elevated eatery that has stylish decor.`,
                img: `https://media-cdn.tripadvisor.com/media/photo-s/19/c6/23/8e/caption.jpg`,
                type: [
                    `Mexican Restaurant`,
                    `Experience`
                ],
                rating: 4.2
            },
            {
                name: `Armani/Ristorante`,
                address: `Burj Khalifa - Lobby Level, Armani Hotel Dubai - Sheikh Mohammed bin Rashid Blvd - Downtown Dubai - Dubai - United Arab Emirates`,
                website: `https://www.armanihotels.com/en/restaurant/armani-ristorante/?hotel=armani-hotel-dubai`,
                description: `Contemporary Italian restaurant in hotel with chic decor, a bar, tasting menu and city views.`,
                img: `https://resizer.otstatic.com/v2/photos/xlarge/1/24885358.jpg`,
                type: [
                    `Italian Restaurant`,
                    `Contemporary`
                ],
                rating: 4.4
            },
            {
                name: `11 Woodfire`,
                address: `Villa 11 75B St - Jumeirah - Jumeirah 1 - Dubai - United Arab Emirates`,
                website: `https://11woodfire.com/?utm_source=GoogleBusinessProfiles&utm_medium=Website&utm_campaign=MapLabs`,
                description: `Produce and ingredients that are carefully selected globally and locally from premium and responsible vendors.`,
                img: `https://www.timeoutdubai.com/cloud/timeoutdubai/2022/08/11/11-Woodfire-1024x768.jpg`,
                type: [
                    `Fine dining`,
                    `Steakhouse`
                ],
                rating: 4.2
            }
        ]
    }
])